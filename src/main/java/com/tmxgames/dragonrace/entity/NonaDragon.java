...
public class NonaDragon extends Animal {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final EntityDataAccessor<Boolean> DATA_BABY_ID = SynchedEntityData.defineId(NonaDragon.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Integer> DATA_PHASE = SynchedEntityData.defineId(NonaDragon.class, EntityDataSerializers.INT);
    ...
    private final NonaDragonPart[] subEntities;
    public final NonaDragonPart head;
    private final NonaDragonPart neck;
    private final NonaDragonPart body;
    private final NonaDragonPart tail1;
    private final NonaDragonPart tail2;
    private final NonaDragonPart tail3;
    private final NonaDragonPart wing1;
    private final NonaDragonPart wing2;
    ...
    @Nullable
    private final NonaDragonPhaseManager phaseManager;
    ...
    public NonaDragon(EntityType<? extends NonaDragon> p_31096_, Level p_31097_) {
        super(ModEntities.NONA_DRAGON.get(), p_31097_);
        this.head = new NonaDragonPart(this, "head", 1.0F, 1.0F);
        this.neck = new NonaDragonPart(this, "neck", 3.0F, 3.0F);
        this.body = new NonaDragonPart(this, "body", 5.0F, 3.0F);
        this.tail1 = new NonaDragonPart(this, "tail", 2.0F, 2.0F);
        this.tail2 = new NonaDragonPart(this, "tail", 2.0F, 2.0F);
        this.tail3 = new NonaDragonPart(this, "tail", 2.0F, 2.0F);
        this.wing1 = new NonaDragonPart(this, "wing", 4.0F, 2.0F);
        this.wing2 = new NonaDragonPart(this, "wing", 4.0F, 2.0F);
        this.subEntities = new NonaDragonPart[]{this.head, this.neck, this.body, this.tail1, this.tail2, this.tail3, this.wing1, this.wing2};
        this.setHealth(this.getMaxHealth());
        this.noPhysics = true;
        this.noCulling = true;

        this.phaseManager = new NonaDragonPhaseManager(this);
        ...
        if (this.isDeadOrDying()) {
            ...
            if (this.isNoAi()) {
                ...
                if (this.level.isClientSide) {
                    ...
                    NonaDragonPhaseInstance nonaDragonPhaseInstance = this.phaseManager.getCurrentPhase();
                    nonaDragonPhaseInstance.doServerTick();
                    if (this.phaseManager.getCurrentPhase() != nonaDragonPhaseInstance) {
                        nonaDragonPhaseInstance = this.phaseManager.getCurrentPhase();
                        nonaDragonPhaseInstance.doServerTick();
                    }

                    Vec3 vec31 = nonaDragonPhaseInstance.getFlyTargetLocation();
                    if (vec31 != null) {
                        double d7 = vec31.x - this.getX();
                        double d8 = vec31.y - this.getY();
                        double d9 = vec31.z - this.getZ();
                        double d3 = d7 * d7 + d8 * d8 + d9 * d9;
                        float f5 = nonaDragonPhaseInstance.getFlySpeed();
                        ...
                        if (Math.abs(d7) > (double)1.0E-5F || Math.abs(d9) > (double)1.0E-5F) {
                            ...
                            this.yRotA *= 0.8F;
                            this.yRotA += f7 * nonaDragonPhaseInstance.getTurnSpeed();
                ...
                for(int k = 0; k < 3; ++k) {
                    NonaDragonPart NonaDragonPart = null;
                    if (k == 0) {
                        NonaDragonPart = this.tail1;
                    }

                    if (k == 1) {
                        NonaDragonPart = this.tail2;
                    }

                    if (k == 2) {
                        NonaDragonPart = this.tail3;
                    }
                    ...
                    this.tickPart(NonaDragonPart, (double)(-(f2 * 1.5F + f18 * f22) * f13), adouble1[1] - adouble[1] - (double)((f22 + 1.5F) * f1) + 1.5D, (double)((f15 * 1.5F + f20 * f22) * f13));
                }
    ...
    private void tickPart(NonaDragonPart p_31116_, double p_31117_, double p_31118_, double p_31119_) {
    ...
    public boolean hurt(NonaDragonPart p_31121_, DamageSource p_31122_, float p_31123_) {
        if (this.phaseManager.getCurrentPhase().getPhase() == NonaDragonPhase.DYING) {
            return false;
        } else {
            ...
            if (p_31123_ < 0.01F) {
                ...
                if (p_31122_.getEntity() instanceof Player || p_31122_.isExplosion()) {
                    ...
                    if (this.isDeadOrDying() && !this.phaseManager.getCurrentPhase().isSitting()) {
                        this.setHealth(1.0F);
                        this.phaseManager.setPhase(NonaDragonPhase.DYING);
                    }
                    ...
                    if (this.phaseManager.getCurrentPhase().isSitting()) {
                        ...
                        if (this.sittingDamageReceived > 0.25F * this.getMaxHealth()) {
                            ...
                            this.phaseManager.setPhase(NonaDragonPhase.TAKEOFF);
                        }
                    }
                }
    ...
    public void readAdditionalSaveData(CompoundTag p_31134_) {
        ...
        if (p_31134_.contains("DragonPhase")) {
            this.phaseManager.setPhase(NonaDragonPhase.getById(p_31134_.getInt("DragonPhase")));
        }
    ...
    public NonaDragonPart[] getSubEntities() {
    ...
    public float getHeadPartYOffset(int p_31109_, double[] p_31110_, double[] p_31111_) {
        NonaDragonPhaseInstance nonaDragonPhaseInstance = this.phaseManager.getCurrentPhase();
        NonaDragonPhase<? extends NonaDragonPhaseInstance> nonaDragonPhase = nonaDragonPhaseInstance.getPhase();
        double d0;
        if (nonaDragonPhase != NonaDragonPhase.LANDING && nonaDragonPhase != NonaDragonPhase.TAKEOFF) {
            if (nonaDragonPhaseInstance.isSitting()) {
                d0 = (double)p_31109_;
            } else if (p_31109_ == 6) {
                d0 = 0.0D;
            } else {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob p_146744_) {
        return ModEntities.NONA_DRAGON.get().create(serverLevel);
    }
    ...
    protected void defineSynchedData() {
        ...
        this.getEntityData().define(DATA_PHASE, NonaDragonPhase.HOVERING.getId());
        this.getEntityData().define(DATA_BABY_ID, true);
    }

    public void onSyncedDataUpdated(EntityDataAccessor<?> entityDataAccessor) {
        if (DATA_PHASE.equals(entityDataAccessor) && this.level.isClientSide) {
            this.phaseManager.setPhase(NonaDragonPhase.getById(this.getEntityData().get(DATA_PHASE)));
        }

        if (DATA_BABY_ID.equals(entityDataAccessor) && this.level.isClientSide) {

        }
        super.onSyncedDataUpdated(entityDataAccessor);
    }

    public NonaDragonPhaseManager getPhaseManager() {
    ...
    public void recreateFromPacket(ClientboundAddEntityPacket p_218825_) {
        ...
        NonaDragonPart[] aNonaDragonPart = this.getSubEntities();

        for(int i = 0; i < aNonaDragonPart.length; ++i) {
            aNonaDragonPart[i].setId(i + p_218825_.getId());
        }
